...
% 谐波专属参数
mu_harm = 5e-4;             
w_harm = zeros(6, 1);       % 2、3、4次谐波
x_harm_buffer = zeros(delay+1, 6);
harm_energy_buffer = zeros(100, 1); % 谐波能量缓冲区
...
for k = 1:n_samples
   ...
   % --- 【修复1：修正变步长逻辑，稳态保留最小跟踪能力】---
    if STEP_MODE == 2
        mu_max_safe = 5e-7;
        mu_min_safe = 1e-7; % 【关键】把稳态最小步长从5e-8提高到1e-7，保留跟踪能力
        % 冷启动 + 频率突变点，强制启用大步长快速收敛
        if t(k) < 1.5 || (t(k) > 9.9 && t(k) < 10.3) || (t(k) > 19.9 && t(k) < 20.3)
            mu_current = mu_max_safe;
        else
            % 稳态反向防御：误差越大，步长越小，但不低于最小步长
    ...
    elseif MODE == 3
        if k >= 200
            ...
            if k >= 2000
                % ============== 【核心修复：先算y_nn_raw，再做平滑，顺序绝对不能反】==============
                % 第一步：先通过蒸馏模型推理，算出y_nn_raw
                y_nn_raw = nn.forward(feature_window); 
                y_nn_raw = y_nn_raw(:); 
                y_nn_raw = y_nn_raw(end); 
                y_nn_raw = double(y_nn_raw) * 0.02; 
                
                % 第二步：再算自适应平滑系数
                err_abs = abs(error_signal(k));
                if err_abs > 3.0
                    alpha_smooth = 0.7; % 大误差/突变期：快响应
                elseif err_abs > 1.0
                    alpha_smooth = 0.8; % 中误差：平衡
                else
                    alpha_smooth = 0.8; % 小误差/稳态：不过度平滑
                end
                
                % 第三步：最后用y_nn_raw更新y_nn(k)
                if k == 2000
                    y_nn(k) = y_nn_raw;
                else
                    y_nn(k) = alpha_smooth * y_nn(k-1) + (1-alpha_smooth) * y_nn_raw; 
                end
                % ====================================================================
            else
                y_nn(k) = 0; 
    ...
    if MIX_MODE == 3 && (MODE == 2 || MODE == 3) && HARM_SWITCH == 1
        ...
        % 【修复4：谐波能量门限，稳态不锁死，保留4次谐波补偿能力】
        harm_energy_buffer = [abs(error_signal(k)); harm_energy_buffer(1:end-1)];
        harm_rms = rms(harm_energy_buffer);
        if (t(k) > 9.9 && t(k) < 10.3) || (t(k) > 19.9 && t(k) < 20.3)
            mu_harm_23 = 8e-4;
            mu_harm_4 = 5e-4; % 突变期4次谐波快速收敛
        elseif harm_rms > 2.0
            mu_harm_23 = 2e-4;
            mu_harm_4 = 1e-4; % 中误差期4次谐波弱更新
        else
            mu_harm_23 = 5e-5; % 【关键】稳态3次谐波学习率从1e-5提高到5e-5，保留跟踪能力
            mu_harm_4 = 2e-5; % 【关键】稳态4次谐波不再锁死为0，保留微小学习率补偿残余
        end
        % 分阶学习率向量 [6, 1]
        mu_harm_vec = [mu_harm_23; mu_harm_23; mu_harm_23; mu_harm_23; mu_harm_4; mu_harm_4];
        
        % 维度安全的权值更新
        gradient_term = error_signal(k) * xf_harm;
        update_term = mu_harm_vec .* gradient_term;
        w_harm = w_harm - update_term;
    else
        y_harm = 0; 
        xf_harm = zeros(length(w_harm), 1); % 维度自适应
    end
    ...
    if MIX_MODE == 3 
        if MODE == 3
            ...
            % 【修复3：提高稳态NN权重下限，强制保留补偿能力】
            if abs(error_signal(k)) > 3.0 || (t(k) > 9.9 && t(k) < 10.3) || (t(k) > 19.9 && t(k) < 20.3)
                mu_wnn = 5e-4; 
                forget_factor = 1.0; 
                w_nn_min = 0.2; % 突变期强制NN主导
            else
                mu_wnn = 1e-5; 
                forget_factor = 0.999; 
                w_nn_min = 0.15; % 【关键】稳态下限从0.1提高到0.15，不让NN完全躺平
            end
            w_nn = forget_factor * w_nn - mu_wnn * error_signal(k) * y_nn_f;
            w_nn = max(min(w_nn, 2.0), w_nn_min); % 强制权重下限
        end
    end
