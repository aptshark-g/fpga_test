...
for k = 1:n_samples
    ...
    % 解耦的步长策略
    if STEP_MODE == 2
        % 【防御型平滑 VSS (Defensive VSS)】
        mu_max_safe = 5e-7; % 稳态巡航步长（此值安全）
        mu_min_safe = 5e-8; % 突变保命步长（极限龟缩）
        mu_cold_start = 2.5e-6;

        % 数学逻辑（极度优美）：
        % 误差越大，exp() 越趋近 0，步长瞬间缩减到 5e-8，把权限让给 NN！
        % 误差越小，exp() 越趋近 1，步长恢复到 5e-7，维持极高稳态精度！
        %mu_current = mu_min_safe + (mu_max_safe - mu_min_safe) * exp(-err_env / 2.0);
        if t(k) < 1.5
            % [冷启动加速期]：刚开机前 2 秒，没有历史错误权重的包袱，
            % 直接全速开荒！迅速建立初始线性底座。
            mu_current = mu_max_safe;
        else
            % [稳态防御期]：开机建系完成后，启动防御机制。
            % 遇到突变时误差变大，步长瞬间缩减到 5e-8，把强攻权限让给神经网络！
            mu_current = mu_min_safe + (mu_max_safe - mu_min_safe) * exp(-err_env / 2.0);
        end
    else
        mu_current = mu_fixed;
    ...
    if k >= 200
        ...
        Feature_Log(k, :) = features; % 记录当前 19 维特征，供云端炼丹使用
    else
        features = zeros(1, 19);
    end
    ...
    if MODE == 2 % 纯边缘在线模型
        ...
        % 对神经元特征进行次级路径滤波 (FxBP 算法核心)
        nn_features_buffer = [nn_features'; nn_features_buffer(1:end-1, :)];
        nn_features_f = (S_est' * nn_features_buffer)'; % 滤波后的特征梯度
        
    elseif MODE == 3 % 云端蒸馏模型
        if k >= 200
            features = extract_features(vibration_raw, error_signal, control_signal, xf_buffer, k, fs, current_f);
            Feature_Log(k, :) = features; 
            
            if k >= 2000
                % 【物理降维打击】：修正云端 Target 的 DC 增益谬误！
                % 将模型输出的标称巨型电压，衰减 50 倍，匹配机床真实的共振放大率。
    ...
    if MODE == 1
        ...
        elseif MIX_MODE == 3
            ...
            %history_factor(k) = rho_gain;
            history_factor(k) = w_nn; % 画图用的依然是它
        end
    end
